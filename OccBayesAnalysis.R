...
#Answer: This dataset represents observations from multiple sites across different years with three different observers.  
#
...
## Answer: There are moderate positive correlations between crop and forests (0.44) could be concerning and indicate overlap or wrong identification of the data. Aquatic and shrub have strong negative correlation (-0.50). Aquatic and ah have a moderate positive correlation. Including these variables in the same model could distort the results of the analysis.  
###
...
# Answer: Crop data does not look very reliable and has large gaps. Forest data is similar, very little data and large gaps. lo data is also sparse and contains large gaps.
#
...
X <- habdf[,c("shrub", "ah", "aquatic", "grass" ) ]
...
#Answer: I would use the wind vector because the data contains less gaps than the cloud coverage. We could be ignoring the time of day/year when this data is collected and how that could be correlated to weather vectors. Precipitation could potentially play a role as well that is not being considered. 
# 

#we create a siteXsurvey matrix for each one and standardize them
#wind
