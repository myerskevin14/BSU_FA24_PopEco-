...
#annotate the code below to make sense of the functions 
...
Time <- opendf %>%  
  ...
  pivot_wider( names_from = year, #which criteria is getting turned to wide: Year and first or second visit
...
# Let's start with a simple model (intercept only)
fm.0 <- pcountOpen( #lambda formula for initial abundance:
  ...
  K = 80, #larger K value takes longer to run 
...
# Answer: Based on the overlap of the 95% CIs for the predictor coefficients, sagebrush and Feb.Min. Temps are significant for gamma and don't overlap zero. Time was also significant for detection probability, with CIs that did not overlap zero. Intercepts for omega and lambda did not cross zero and can be considered important for the responses. 
#
...
sage <- seq( min( opendf[,"sagebrush"]),max( opendf[,"sagebrush"]),
             length.out = n ) #evenly creates 100 values of the spread 
#standardize them
sage.std <- scale( sage )
...
# Answer: There is a positive relationship between the percentage of sagebrush cover and recruitment. As sagebrush coverage increase, recruitment increases. 
#
...
#view
maxTp
# Add plots for detection (time)
# Answer: There is a significant negative relationship between the Maximum temperature in April and May with recruitment. As maximum temperatures increas during these months, recruitment decreases. 
#
############################################################################
