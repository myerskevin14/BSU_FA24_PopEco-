...
#answer: The second model (m2) because it has more robust parameter estimates even though all models are fairly similar.

#get summary from three models:
summary(m1); summary(m2); summary(m3)

# For homework you don't have to compare models since you only run one. # I chose model 2
...
# Label columns based on the order of predictors in the detection model
colnames(alpha.mat) <- c("Time", "Time^2") # adjust based on your actual predictors

# Convert matrix to long format
alpha.df <- tidyr::gather(data = as.data.frame(alpha.mat), key = Predictor, Value)

# Plot detection submodel
...
# unmarked analysis?
# Answer: This plot shows that as sagebrush coverage increases, the relative abundance of squirrels increases. The other plot from the unmarked analysis (CountAnalysis.R) shows that as time after 6am increases, probability of occupancy decreases after 150 minutes past 6am. 
# 
...
#How does this plot compare to the one you obtained from the 
# unmarked analysis?
# Answer: This plot shows a very similar trend. As time past 6am increases, probability of detection significantly decreases after 150 minutes past 6am. Both plots show this trend but have different y axis values. The plot in this script shows that detection is imperfect, which is not uncommon. Estimates of occupancy could be biased to show the downward trend because of this detection trend. 
# 
...
#Code here: 
save.image( "CountBayesPlotting.RData" )
###
####################### end of script #################################