...
opendf <-  obs_df %>% 
  ...
  dplyr::filter( counted == "yes" ) %>% #filter function filters rows
  #select columns to keep:
  dplyr::select( o.sites, counted, year, count.j1, count.j2, count.j3, #select function filters columns 
                 observer.j1, observer.j2, observer.j3,
                 time.j1, time.j2, time.j3 ) 
#check
head( opendf ); dim( opendf ) #only yes values included 
...
# Answer: Right_joint because we want to keep all of the rows from the open data frame and add rows from our preddf. 
#
...
T <- length( yrrange ) # T for time
# How many replicate surveys
J <- 3 #J for survey
#view
I; yrrange; T; J #I for site
...
# site occupancy? 
# Answers: Site abundances are fairly consistent over time, with some sites having large booms in 2013 but then dropping back to around the average after time. 
#
